...
void test_quat_inverse(void) {
  {
    ...
    }
    {
      ...
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, -0.35355334825f, combined.x);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, 0.85355333052f, combined.w);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, -0.35355334825f, combined.y);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, -0.14644658464f, combined.z);

      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, 0.853553475357717f, inverse.w);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, 0.353553408243744f, inverse.x);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, 0.353553408243744f, inverse.y);
      TEST_ASSERT_FLOAT_WITHIN(FLT_EPSILON, 0.146446609490221f, inverse.z);
